    ...
    while (!orphan_work_set.empty()) {
        ...
        if (AcceptToMemoryPool(m_mempool, state, porphanTx, &removed_txn, false /* bypass_limits */)) {
            ...
            if (state.GetResult() != TxValidationResult::TX_WITNESS_STRIPPED) {
                ...
                // https://github.com/interchained/interchained/issues/8279 for details.
                // We can remove this restriction (and always add wtxids to
                // the filter even for witness stripped transactions) once
                // wtxid-based relay is broadly deployed.
                // See also comments in https://github.com/interchained/interchained/pull/18044#discussion_r443419034
    ...
    if (msg_type == NetMsgType::CMPCTBLOCK)
    {
        ...
        if (pindex->nHeight <= ::ChainActive().Height() + 2) {
            if ((!fAlreadyInFlight && nodestate->nBlocksInFlight < MAX_BLOCKS_IN_TRANSIT_PER_PEER) ||
                ...
                status = tempBlock.FillBlock(*pblock, dummy, pindex);
                if (status == READ_STATUS_OK) {
