...
static CBlock BuildBlockTestCase() {
    ...
    assert(!mutated);
    while (!CheckProofOfWork(block.GetHash(), block, block.nBits, Params().GetConsensus(), 0)) ++block.nNonce;
    return block;
}
...
BOOST_AUTO_TEST_CASE(SimpleRoundTripTest)
{
    ...
    // Do a simple ShortTxIDs RT
    {
        ...
        CBlock block2;
        {
            ...
            BOOST_CHECK(partialBlock.FillBlock(block2, {}, pindex) == READ_STATUS_INVALID); // No transactions
            partialBlock = tmp;
        }
        ...
        // Wrong transaction
        {
            PartiallyDownloadedBlock tmp = partialBlock;
            const CBlockIndex* pindex = nullptr;
            partialBlock.FillBlock(block2, {block.vtx[2]}, pindex); // Current implementation doesn't check txn here, but don't require that
            partialBlock = tmp;
        }
        bool mutated;
        ...
        CBlock block3;
        const CBlockIndex* pindex = nullptr;
        BOOST_CHECK(partialBlock.FillBlock(block3, {block.vtx[1]}, pindex) == READ_STATUS_OK);
...
BOOST_AUTO_TEST_CASE(NonCoinbasePreforwardRTTest)
{
    ...
    // Test with pre-forwarding tx 1, but not coinbase
    {
        ...
        // Wrong transaction
        {
            PartiallyDownloadedBlock tmp = partialBlock;
            const CBlockIndex* pindex = nullptr;
            partialBlock.FillBlock(block2, {block.vtx[1]}, pindex); // Current implementation doesn't check txn here, but don't require that
            partialBlock = tmp;
        }
...
BOOST_AUTO_TEST_CASE(SufficientPreforwardRTTest)
{
    ...
    // Test with pre-forwarding coinbase + tx 2 with tx 1 in mempool
    {
        ...
        PartiallyDownloadedBlock partialBlockCopy = partialBlock;
        const CBlockIndex* pindex = nullptr;
        BOOST_CHECK(partialBlock.FillBlock(block2, {}, pindex) == READ_STATUS_OK);
...
BOOST_AUTO_TEST_CASE(EmptyBlockRoundTripTest)
{
    ...
    // Test simple header round-trip with only coinbase
    {
        ...
        const CBlockIndex* pindex = nullptr;
        BOOST_CHECK(partialBlock.FillBlock(block2, vtx_missing, pindex) == READ_STATUS_OK);
        BOOST_CHECK_EQUAL(block.GetHash().ToString(), block2.GetHash().ToString());
        BOOST_CHECK_EQUAL(block.hashMerkleRoot.ToString(), BlockMerkleRoot(block2, &mutated).ToString());
        BOOST_CHECK(!mutated);
    }
