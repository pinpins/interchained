...
static RPCHelpMan getnewaddress()
{
    return RPCHelpMan{"getnewaddress",
                "\nReturns a new Interchained address for receiving payments.\n"
                ...
                RPCResult{
                    RPCResult::Type::STR, "address", "The new interchained address"
                },
                RPCExamples{
...
static RPCHelpMan getrawchangeaddress()
{
    return RPCHelpMan{"getrawchangeaddress",
                "\nReturns a new Interchained address, for receiving change.\n"
...
static RPCHelpMan setlabel()
{
    return RPCHelpMan{"setlabel",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The interchained address to be associated with a label."},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!IsValidDestination(dest)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Interchained address");
    }
...
void ParseRecipients(const UniValue& address_amounts, const UniValue& subtract_fee_outputs, std::vector<CRecipient> &recipients) {
    ...
    for (const std::string& address: address_amounts.getKeys()) {
        ...
        if (!IsValidDestination(dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Interchained address: ") + address);
        }
        ...
        HELP_REQUIRING_PASSPHRASE,
                {
                    ...
                    {"subtractfeefromamount", RPCArg::Type::BOOL, /* default */ "false", "The fee will be deducted from the amount being sent.\n"
                                         "The recipient will receive less interchaineds than you enter in the amount field."},
            ...
            "       \"" + FeeModes("\"\n\"") + "\""},
                ...
                RPCExamples{
                    ...
                    "\nSend 0.1 ITC with a confirmation target of 6 blocks in economical fee estimate mode using positional arguments\n"
                    + HelpExampleCli("sendtoaddress", "\"" + EXAMPLE_ADDRESS[0] + "\" 0.1 \"donation\" \"sean's outpost\" false true 6 economical") +
                    "\nSend 0.1 ITC with a fee rate of 1.1 " + CURRENCY_ATOM + "/vB, subtract fee from amount, BIP125-replaceable, using positional arguments\n"
                    + HelpExampleCli("sendtoaddress", "\"" + EXAMPLE_ADDRESS[0] + "\" 0.1 \"drinks\" \"room77\" true true null \"unset\" null 1.1") +
                    "\nSend 0.2 ITC with a confirmation target of 6 blocks in economical fee estimate mode using named arguments\n"
                    + HelpExampleCli("-named sendtoaddress", "address=\"" + EXAMPLE_ADDRESS[0] + "\" amount=0.2 conf_target=6 estimate_mode=\"economical\"") +
                    "\nSend 0.5 ITC with a fee rate of 25 " + CURRENCY_ATOM + "/vB using named arguments\n"
                ...
                RPCResult{
                    RPCResult::Type::ARR, "", "",
                    {
                        {RPCResult::Type::ARR, "", "",
                        {
                            {RPCResult::Type::ARR, "", "",
                            {
                                {RPCResult::Type::STR, "address", "The interchained address"},
...
static RPCHelpMan signmessage()
{
    return RPCHelpMan{"signmessage",
        ...
        HELP_REQUIRING_PASSPHRASE,
        {
            {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The interchained address to use for the private key."},
    ...
    [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
    {
        ...
        // Use LegacyScriptPubKeyMan to access the SigningProvider
        LegacyScriptPubKeyMan* spk_man = pwallet->GetLegacyScriptPubKeyMan();
        if (!spk_man) {
            throw JSONRPCError(RPC_WALLET_ERROR, "Legacy script pubkey manager not available.");
        }
        const SigningProvider& provider = *spk_man;
        ...
        if (const PKHash* pkhash = boost::get<PKHash>(&dest)) {
            ...
            CKeyID keyID = CKeyID(uint160(*wpkh));
            if (!provider.GetKey(keyID, key)) {
                throw JSONRPCError(RPC_WALLET_ERROR, "Private key not available");
            }
            pubkey = key.GetPubKey();
            ...
            throw JSONRPCError(RPC_TYPE_ERROR, "Address does not refer to a supported key type");
        }

        if (!key.IsValid()) {
            throw JSONRPCError(RPC_WALLET_ERROR, "Private key is invalid");
        }

        std::string signature;
        SigningResult err = pwallet->SignMessage(strMessage, PKHash(pubkey), signature);
...
static RPCHelpMan getreceivedbyaddress()
{
    return RPCHelpMan{"getreceivedbyaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The interchained address for transactions."},
        ...
        HELP_REQUIRING_PASSPHRASE,
                {
                    ...
                    {"amounts", RPCArg::Type::OBJ, RPCArg::Optional::NO, "The addresses and amounts",
                        {
                            {"address", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "The interchained address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value"},
                        },
                    },
                    ...
                    {"subtractfeefrom", RPCArg::Type::ARR, RPCArg::Optional::OMITTED_NAMED_ARG, "The addresses.\n"
                                       ...
                                       "Those recipients will receive less interchaineds than you enter in their corresponding amount field.\n"
...
static RPCHelpMan addmultisigaddress()
{
    return RPCHelpMan{"addmultisigaddress",
                ...
                "Each key is a Interchained address or hex-encoded public key.\n"
                "This functionality is only intended for use with non-watchonly addresses.\n"
                "See `importaddress` for watchonly p2sh address support.\n"
                "If 'label' is specified, assign address to that label.\n",
                {
                    {"nrequired", RPCArg::Type::NUM, RPCArg::Optional::NO, "The number of required signatures out of the n keys or addresses."},
                    {"keys", RPCArg::Type::ARR, RPCArg::Optional::NO, "The interchained addresses or hex-encoded public keys",
                        {
                            {"key", RPCArg::Type::STR, RPCArg::Optional::OMITTED, "interchained address or hex-encoded public key"},
                        },
                        },
    ...
    return RPCHelpMan{"listtransactions",
                ...
                RPCResult{
                    RPCResult::Type::ARR, "", "",
                    {
                        ...
                        {
                            {RPCResult::Type::BOOL, "involvesWatchonly", "Only returns true if imported addresses were involved in transaction."},
                            {RPCResult::Type::STR, "address", "The interchained address of the transaction."},
                ...
                RPCResult{
                    ...
                    TransactionDescriptionString()),
                    {
                        {RPCResult::Type::ARR, "details", "",
                        {
                            ...
                            {
                                {RPCResult::Type::BOOL, "involvesWatchonly", "Only returns true if imported addresses were involved in transaction."},
                                {RPCResult::Type::STR, "address", "The interchained address involved in the transaction."},
                                {RPCResult::Type::STR, "category", "The transaction category.\n"
                                    "\"send\"                  Transactions sent.\n"
                                    "\"receive\"               Non-coinbase transactions received.\n"
                                    "\"generate\"              Coinbase transactions received with more than 100 confirmations.\n"
...
static RPCHelpMan walletpassphrase()
{
    return RPCHelpMan{"walletpassphrase",
                ...
                "This is needed prior to performing transactions related to private keys such as sending interchaineds\n"
            "\nNote:\n"
...
static RPCHelpMan encryptwallet()
{
    return RPCHelpMan{"encryptwallet",
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending interchained\n"
...
static RPCHelpMan lockunspent()
{
    return RPCHelpMan{"lockunspent",
                ...
                "A locked transaction output will not be chosen by automatic coin selection, when spending interchaineds.\n"
...
static RPCHelpMan loadwallet()
{
    return RPCHelpMan{"loadwallet",
                ...
                "\nNote that all wallet command-line options used when starting interchainedd will be"
...
static RPCHelpMan listunspent()
{
    return RPCHelpMan{
                ...
                "Optionally filter to only include txouts paid to specified addresses.\n",
                {
                    ...
                    {"addresses", RPCArg::Type::ARR, /* default */ "empty array", "The interchained addresses to filter",
                        {
                            {"address", RPCArg::Type::STR, RPCArg::Optional::OMITTED, "interchained address"},
                        },
                    },
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Interchained address: ") + input.get_str());
            }
    ...
    if (!options.isNull()) {
      if (options.type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress") || options.exists("change_address")) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Change address must be a valid interchained address");
            }
                    ...
                    {"options", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED_NAMED_ARG, "for backward compatibility: passing in a true instead of an object will result in {\"includeWatching\":true}",
                        {
                            ...
                            {"changeAddress", RPCArg::Type::STR, /* default */ "pool address", "The interchained address to receive the change"},
        ...
        [want_psbt](const RPCHelpMan& self, const JSONRPCRequest& request) mutable -> UniValue
{
    ...
    if (pwallet->IsWalletFlagSet(WALLET_FLAG_DISABLE_PRIVATE_KEYS) && !want_psbt) {
        if (!pwallet->chain().rpcEnableDeprecated("bumpfee")) {
            throw JSONRPCError(RPC_METHOD_DEPRECATED, "Using bumpfee with wallets that have private keys disabled is deprecated. Use psbtbumpfee instead or restart interchainedd with -deprecatedrpc=bumpfee. This functionality will be removed in 0.22");
        }
        want_psbt = true;
    }
...
RPCHelpMan getaddressinfo()
{
    return RPCHelpMan{"getaddressinfo",
                "\nReturn information about the given interchained address.\n"
                "Some of the information will only be present if the address is in the active wallet.\n",
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The interchained address for which to get information."},
                },
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
                        {RPCResult::Type::STR, "address", "The interchained address validated."},
...
static RPCHelpMan send()
{
    return RPCHelpMan{"send",
        ...
        "\nSend a transaction.\n",
        {
            {"outputs", RPCArg::Type::ARR, RPCArg::Optional::NO, "The outputs (key-value pairs), where none of the keys are duplicated.\n"
                    ...
                    "For convenience, a dictionary, which holds the key-value pairs directly, is also accepted.",
                {
                    {"", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED, "",
                        {
                            {"address", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "A key-value pair. The key (string) is the interchained address, the value (float or string) is the amount in " + CURRENCY_UNIT + ""},
                        },
                        },
                    {"", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED, "",
                        {
            ...
            {"options", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED_NAMED_ARG, "",
                {
                    ...
                    {"change_address", RPCArg::Type::STR_HEX, /* default */ "pool address", "The interchained address to receive the change"},
        ...
        RPCExamples{""
        "\nSend 0.1 ITC with a confirmation target of 6 blocks in economical fee estimate mode\n"
        + HelpExampleCli("send", "'{\"" + EXAMPLE_ADDRESS[0] + "\": 0.1}' 6 economical\n") +
        "Send 0.2 ITC with a fee rate of 1.1 " + CURRENCY_ATOM + "/vB using positional arguments\n"
        + HelpExampleCli("send", "'{\"" + EXAMPLE_ADDRESS[0] + "\": 0.2}' null \"unset\" 1.1\n") +
        "Send 0.2 ITC with a fee rate of 1 " + CURRENCY_ATOM + "/vB using the options argument\n"
        + HelpExampleCli("send", "'{\"" + EXAMPLE_ADDRESS[0] + "\": 0.2}' null \"unset\" null '{\"fee_rate\": 1}'\n") +
        "Send 0.3 ITC with a fee rate of 25 " + CURRENCY_ATOM + "/vB using named arguments\n"
    ...
    return RPCHelpMan{"walletcreatefundedpsbt",
                ...
                "Implements the Creator and Updater roles.\n",
                {
                    ...
                    {"options", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED_NAMED_ARG, "",
                        {
                            {"add_inputs", RPCArg::Type::BOOL, /* default */ "false", "If inputs are specified, automatically include more if they are not enough."},
                            {"changeAddress", RPCArg::Type::STR_HEX, /* default */ "pool address", "The interchained address to receive the change"},
                            {"changePosition", RPCArg::Type::NUM, /* default */ "random", "The index of the change output"},
                            {"change_type", RPCArg::Type::STR, /* default */ "set by -changetype", "The output type to use. Only valid if changeAddress is not specified. Options are \"legacy\", \"p2sh-segwit\", and \"bech32\"."},
                            {"includeWatching", RPCArg::Type::BOOL, /* default */ "true for watch-only wallets, otherwise false", "Also select inputs which are watch only"},
                            {"lockUnspents", RPCArg::Type::BOOL, /* default */ "false", "Lock selected unspent outputs"},
...
static std::string FormatTokenAmount(const CAmount& amount, uint8_t decimals) {
    if (decimals > 16) decimals = 16;
    std::string str = strprintf("%.*f", decimals, amount / std::pow(10.0, decimals));
...
static RPCHelpMan createtoken()
{
    return RPCHelpMan{
        ...
        {
            {"amount", RPCArg::Type::STR, RPCArg::Optional::NO, "Amount of tokens to create (use string to preserve decimal precision)"},
        ...
        RPCResult{
            RPCResult::Type::OBJ, "", "",
            {
                {RPCResult::Type::BOOL, "success", "True if the token was created successfully"},
                {RPCResult::Type::STR, "token_id", "Generated token identifier"}
            },
        },
        RPCExamples{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            if (decimalsInt < 0 || decimalsInt > 16) {
                throw JSONRPCError(RPC_INVALID_PARAMETER, "Decimals must be between 0 and 16");
            }

            uint8_t decimals = static_cast<uint8_t>(decimalsInt);

            CAmount rawAmount;
            if (!ParseFixedPoint(amountStr, decimals, &rawAmount)) {
            ...
            result.pushKV("token_id", token_id);
            return result;
        }
    };
}
...
static RPCHelpMan tokenallowance()
{
    return RPCHelpMan{
        ...
        {
            {"owner", RPCArg::Type::STR, RPCArg::Optional::NO, "Owner address"},
            {"spender", RPCArg::Type::STR, RPCArg::Optional::NO, "Spender address"},
            {"token", RPCArg::Type::STR, RPCArg::Optional::NO, "Token identifier"},
        },
        RPCResult{
            RPCResult::Type::STR,
            "",
            "Remaining allowance (formatted string)"
        },
        RPCExamples{
...
static RPCHelpMan tokentransferfrom()
{
    return RPCHelpMan{
        ...
        {
            {"from", RPCArg::Type::STR, RPCArg::Optional::NO, "Source address"},
            {"to", RPCArg::Type::STR, RPCArg::Optional::NO, "Destination address"},
            {"token", RPCArg::Type::STR, RPCArg::Optional::NO, "Token identifier"},
            {"amount", RPCArg::Type::STR, RPCArg::Optional::NO, "Amount to transfer (string to preserve decimal precision)"},
            {"memo", RPCArg::Type::STR, RPCArg::Optional::NO, "Optional memo string"},
...
static RPCHelpMan tokenincreaseallowance()
{
    return RPCHelpMan{
        ...
        RPCExamples{
            HelpExampleCli("tokenincreaseallowance", "\"spender\" \"tokenidtok\" \"1.00000001\" false")
        },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            std::shared_ptr<CWallet> const wallet = GetWalletForJSONRPCRequest(request);
...
static RPCHelpMan tokendecreaseallowance()
{
    return RPCHelpMan{
        ...
        RPCExamples{
            HelpExampleCli("tokendecreaseallowance", "\"spender\" \"tokenidtok\" \"1.00000000\" false")
        },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            std::shared_ptr<CWallet> const wallet = GetWalletForJSONRPCRequest(request);
...
static RPCHelpMan tokenburn()
{
    return RPCHelpMan{
        ...
        RPCExamples{
            HelpExampleCli("tokenburn", "\"tokenidtok\" \"1.00000000\" false")
        },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            std::shared_ptr<CWallet> const wallet = GetWalletForJSONRPCRequest(request);
...
static RPCHelpMan tokenmint()
{
    return RPCHelpMan{
        ...
        RPCExamples{
            HelpExampleCli("tokenmint", "\"tokenidtok\" \"1.00000000\" false")
        },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            std::shared_ptr<CWallet> const wallet = GetWalletForJSONRPCRequest(request);
...
static RPCHelpMan tokentransferownership()
{
    return RPCHelpMan{
        ...
        RPCExamples{
            HelpExampleCli("tokentransferownership", "\"tokenidtok\" \"newowner\" false")
        },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            std::shared_ptr<CWallet> const wallet = GetWalletForJSONRPCRequest(request);
...
static RPCHelpMan tokentotalsupply()
{
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            CAmount rawAmount = g_token_ledger.TotalSupply(token_id);
            int decimals = g_token_ledger.GetDecimals(token_id);
    ...
    return RPCHelpMan{
        ...
        RPCResult{
            ...
            "Token metadata object",
            {
                ...
                {RPCResult::Type::NUM, "decimals", "Token decimals"},
                {RPCResult::Type::STR, "creator", "Creator wallet"},
                {RPCResult::Type::NUM, "created_height", "Creation block"},
                {RPCResult::Type::STR, "total_supply", "Total supply (formatted string)"}
            }
        },
        RPCExamples{
...
static RPCHelpMan getgovernancebalance()
{
    return RPCHelpMan{
        ...
        {},
        RPCResult{
            RPCResult::Type::OBJ, "", "Object containing the governance balance",
            {
                {RPCResult::Type::STR, "amount", "Fee amount (formatted string)"}
            }
        },
        RPCExamples{
...
static RPCHelpMan my_tokens()
{
    return RPCHelpMan{
        ...
        RPCResult{
            RPCResult::Type::ARR,
            "",
            "Array of tokens held by this wallet",
            {
                RPCResult{
                    RPCResult::Type::OBJ,
                    "",
                    "Token metadata",
                    {
                        {RPCResult::Type::STR, "symbol", "Token symbol"},
                        {RPCResult::Type::STR, "name", "Token name"},
                        {RPCResult::Type::STR, "address", "Token ID"},
                        {RPCResult::Type::STR, "balance", "Formatted token balance string"}
                    }
                }
            }
        },
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            for (const auto& item : list) {
                const std::string& token_id = std::get<0>(item);
                const std::string& name = std::get<1>(item);
                const std::string& symbol = std::get<2>(item);

                CAmount balance = g_token_ledger.GetBalance(signer, token_id);
                if (balance <= 0) continue;

                int decimals = g_token_ledger.GetDecimals(token_id);
                std::string formatted = FormatTokenAmount(balance, decimals);

                UniValue obj(UniValue::VOBJ);
                obj.pushKV("symbol", symbol);
                obj.pushKV("name", name);
                obj.pushKV("address", token_id);
                obj.pushKV("balance", formatted);
                arr.push_back(obj);
            }
    ...
    return RPCHelpMan{
        ...
        RPCResult{
            ...
            "Scan result object",
            {
                ...
                {RPCResult::Type::NUM, "stop_height", "Last scanned height"}
            }
        },
        RPCExamples{
