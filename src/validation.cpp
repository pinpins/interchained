...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (!m_pool.CalculateMemPoolAncestors(*entry, setAncestors, m_limit_ancestors, m_limit_ancestor_size, m_limit_descendants, m_limit_descendant_size, errString)) {
        ...
        // this, see https://lists.linuxfoundation.org/pipermail/interchained-dev/2018-November/016518.html
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    if (nHeight <= rampUpEnd) {
        // Linear ramp-up: 0.678 to 1.5 ITC
        double progress = static_cast<double>(nHeight) / rampUpEnd;
        reward = 0.678 + (1.5 * progress); // 0.5 → 1.5
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (pindex->nHeight >= 1 && IsValidDestination(opDest)) {
        ...
        CAmount expectedOp = (blockReward * OPERATOR_BPS) / BPS_DENOM;
        bool foundOp = false;
        for (const auto& out : block.vtx[0]->vout) {
    ...
    if (burn_fees) {
        chainparams.AddBurnedFees(nFees);
    }
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (nHeight >= consensusParams.difficultyForkHeight) {
        const int64_t clampLimit = nMedianTimePast + 240 * 60; // 240 minutes
        if (block.GetBlockTime() > clampLimit) {
            return state.Invalid(BlockValidationResult::BLOCK_TIME_FUTURE, "time-too-new", 
                strprintf("⛔️ Block timestamp too far in future: nTime=%d > MTP+240min=%d", block.GetBlockTime(), clampLimit));
        }
    } else {
            ...
            try {
                ...
                while (!queue.empty()) {
                    ...
                    while (range.first != range.second) {
                        ...
                        const CBlockIndex* pindex = LookupBlockIndex(pblockrecursive->GetHash());
                        if (pindex && ReadBlockFromDisk(*pblockrecursive, it->second, chainparams.GetConsensus(), pindex->nHeight))
                        {
