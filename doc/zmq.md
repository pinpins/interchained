...
The Interchained Core daemon can be configured to act as a trusted "border
router", implementing the interchained wire protocol and relay, making
...
The ZeroMQ feature in Interchained Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building interchainedd:
...
For instance:
    ...
    $ interchainedd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/interchainedd.tx.raw \
...
These options can also be provided in interchained.conf.
...
From the perspective of interchainedd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into interchainedd directly. Furthermore, no information is
...
using. Interchainedd appends an up-counting sequence number to each
