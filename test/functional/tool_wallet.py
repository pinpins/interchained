...
class ToolWalletTest(InterchainedTestFramework):
    ...
    def interchained_wallet_process(self, *args):
        binary = self.config["environment"]["BUILDDIR"] + '/src/interchained-wallet' + self.config["environment"]["EXEEXT"]
        args = ['-datadir={}'.format(self.nodes[0].datadir), '-chain=%s' % self.chain] + list(args)
        return subprocess.Popen([binary] + args, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)

    def assert_raises_tool_error(self, error, *args):
        p = self.interchained_wallet_process(*args)
    ...
    def test_invalid_tool_commands_and_args(self):
        ...
        # `interchained-wallet help` raises an error. Use `interchained-wallet -help`.
        ...
        if self.options.descriptors:
            error = "SQLiteDatabase: Unable to obtain an exclusive lock on the database, is it being used by another interchainedd?"
    ...
    def test_salvage(self):
        # TODO: Check salvage actually salvages and doesn't break things. https://github.com/interchained/interchained/issues/7463
